...
- The active registered tools are `inspect-install`, `list-files`, `list-systems`, `report-system`, `list-entity-systems`, `find-entity`, `describe-entity`, `list-entity-links`, `report-diplomacy-war-flow`, `plan-mod-update`, `apply-mod-update`, and `describe-server`.
...
- Broader cross-entity diplomacy-graph tooling is still future work.
...
The first diplomacy-helper expansion stays similarly constrained. `report-diplomacy-war-flow` reads the representative diplomacy install files already curated by `GameInstall.representative_files()`, then delegates report building to the stable grouped `eu5miner.domains.diplomacy` helper APIs. The MCP layer only shapes the tool contract and serialization.
